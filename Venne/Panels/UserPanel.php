 ...
 *
 * @author Mikuláš Dítě, Josef Kříž <pepakriz@gmail.com>
 * @license MIT
 */
...
class UserPanel extends Control implements IBarPanel {
	...
	public function __construct(\Nette\DI\IContainer $container)
	{
		...
		parent::__construct($container->application->presenter, 'UserPanel');
	...
	public function onLoginSubmitted(Form $form)
	{
		try {
			...
			if ($username == '__guest') {
				...
				$this->container->user->login($username, $password);
			}

			$this->redirect('this');
		} catch (AuthenticationException $e) {
			$this->container->application->presenter->flashMessage($e->getMessage(), 'error');
			$this->redirect('this');
		}
	}
