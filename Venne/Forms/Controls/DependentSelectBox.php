...
// <editor-fold defaultstate="collapsed" desc="constructor, initializing, attaching">
	...
	public function __construct($label, $parents, $dataCallback)
	{
		...
		if (!(is_callable($dataCallback))) throw new InvalidArgumentException("Given callback is not callable !");
		$this->dataCallback = $dataCallback;
		if (!is_array($parents)) $parents = ($parents === null) ? array() : array($parents);
		...
		foreach ($parents as $parent) {
			...
			if ($parent instanceof DependentSelectBox) $parent->childs[] = $this;
			if (!self::$disableChilds && $this->isRoot($parent) && $parent instanceof SelectBox && $parent->getPrompt()) throw new NotSupportedException('When first item on root is skipped, $disableChilds = false cant be used !');
		}
	}
...
// <editor-fold defaultstate="collapsed" desc="getters & setters & add & remove">
	...
	public function setValue($value, $refreshTree = true)
	{
		...
		if ($refreshTree && !$this->hasEmptyValue()) $this->refresh(false);
		return $this;
	}
	...
	public function setDisabledValue($disabledValue = null)
	{
		...
		if ($this->isDependentSelectBoxDisabled()) $this->disableSelectBox();
	}
	...
	{
		if (!$useKeys) throw new NotSupportedException("Working without keys not supported !");
	...
	{
		if (!is_callable($callback)) throw new InvalidArgumentException("Given callback is not callable !");
...
// <editor-fold defaultstate="collapsed" desc="signals">
	...
	public function submitButtonHandler($button)
	{
		...
		foreach ($this->onSubmit as $onSubmitItem) call_user_func_array($onSubmitItem[0], $onSubmitItem[1]);

		if ($this->hasAnyParentEmptyValue()) return;
...
// <editor-fold defaultstate="collapsed" desc="tools & helpers">
	...
	public function refresh($clearValue = false, $recursive = true)
	{
		...
		if ($clearValue) $this->setValue(null, false);
	...
	{
		if ($control instanceof FormControlDependencyHelper) return $control;
		...
		if ($this->autoSelectRootFirstItem) {
			foreach ($this->parents as $parent) {
				if ($this->isRoot($parent) && $parent->getValue() === null) {
					if (!($parent instanceof SelectBox)) throw new InvalidArgumentException("When using 'autoSelectRootFirstItem = true', parent must be instance of SelectBox !");
					$items = $parent->getItems();
					if (!empty($items)) {
						if ($parent->areKeysUsed()) $items = array_keys($items);
	...
	protected function hasAnyParentEmptyValue()
	{
		foreach ($this->parents as $parent) {
			if ($parent instanceof DependentSelectBox) {
				if ($parent->hasEmptyValue()) return true;
			} else {
				if (in_array($parent->getValue(), self::$emptyValues, true)) return true;
			}
		}
		return false;
	}
	...
	protected function setItemsFromCallback($form)
	{
		...
		if (!is_array($data)) throw new InvalidArgumentException("Data must be array !");
		$this->setItems($data);
	...
	protected function disableSelectBox()
	{
		...
		if ($this->getControlPrototype()->class == null) $this->getControlPrototype()->class = "";
	...
	protected function addEmptyHeaderItem($selectFirst = true)
	{
		...
		if ($selectFirst) $this->setFirstItemSelected();
	}
	...
	{
		if (PHP_VERSION_ID >= 50300) FormContainer::extensionMethod($methodName, "DependentSelectBox\DependentSelectBox::Container_prototype_addDependentSelectBox"); else
